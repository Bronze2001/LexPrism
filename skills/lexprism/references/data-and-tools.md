...
| 对象 | 核心字段 |
| --- | --- |
| Task | task_id、目标、业务类型、jurisdictions、as_of、reader、deliverable、source_mode、输入版本 |
...
| Claim | claim_id、issue_id、类型、表述、事实前提、支持及反对passage_id、四层核验状态 |
| Draft | draft_id、version、稳定段落ID、引用claim_id、变更记录、交付状态 |
| Run | run_id、输入／技能／模板版本、步骤状态、工具结果引用、失败、耗时、可得的模型参数 |
...
检索提供方先按 [research.md](research.md) 的“法源分流规则”选择，再匹配真实工具。CN-001 的首选为北大法宝；其他路线随用户补充及 MCP 封装进度逐步映射，不能从规划文档推定工具已连接。
...
如果宿主支持文件保存，使用项目指定位置按task_id/run_id分开存放；原始材料、法源、范例和专家答案分区。日志不记录凭据。外部查询发送解决问题所需的信息，不默认上传整个案件目录。

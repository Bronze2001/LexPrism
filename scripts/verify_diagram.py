...
APPROVED_PALETTE = {
    ...
    # 控股层 / 投资人 (专业深蓝系)
    '#2B6CB0', '#3182CE', '#1E40AF', '#1D4ED8', '#BEE3F8', '#2B6CB0',
    # 普通子公司 / 背景层 (素雅灰白系)
    '#EDF2F7', '#CBD5E0', '#2D3748', '#E2E8F0', '#F8FAFC', '#334155', '#4A5568', '#718096', '#A0AEC0',
    # 通道 / 员工持股平台 (商务橙/黄系)
...
def verify_drawio_file(filepath):
    ...
    for cell in cells:
        ...
        if val:
            # 检查标题 (支持架构图、流向图、时间线、图谱、推导树、报告等)
            if any(kw in val for kw in ('架构图', '流向图', '时间线', '图谱', '推导树', '报告', '关系图', '【LexPrism')):
                has_title = True
